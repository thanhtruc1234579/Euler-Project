...
"""
    ...
    # First. we need to split all of them and convert to the integer type
    numbers = GRID.strip().split('\n')
    for i in range(20):
        numbers[i] = list(map(int, numbers[i].split(' ')))

    max_h = max_v = max_diagonal1 = max_diagonal2 = -1
    ...
    # max_diagonal1 (top left bottom right)
    for r in range(17):
        for c in range(17):
            mul = 1
            for d in range(4):
